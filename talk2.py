...
def chatgpt_streamed(user_input, system_message, conversation_history, bot_name, vault_embeddings, vault_content, model):
    ...
    # Check if the response starts with "Record this", "record this", "Record this.", or "record this."
    if any(phrase in full_response for phrase in ["Record this", "record this", "Record this.", "record this."]):
        info_to_add = full_response.split("Record this.")[-1].strip()
        info_to_add = info_to_add or full_response.split("record this.")[-1].strip()
        info_to_add = info_to_add or full_response.split("Record this,")[-1].strip()
        info_to_add = info_to_add or full_response.split("record this,")[-1].strip()
        if info_to_add:
            with open("vault.txt", "a", encoding="utf-8") as vault_file:
                vault_file.write(info_to_add + "\n")
            print("Added to info.")
...
def user_chatbot_conversation():
    ...
    while True:
        ...
        if chatbot_response.startswith(("Record this", "record this", "Record this.", "record this.")):
            actual_response = chatbot_response[13:].strip()
        else:
            actual_response = chatbot_response
